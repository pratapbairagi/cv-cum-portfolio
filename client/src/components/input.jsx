...
const Input = ({ type, placeholder, defaultValue, func, css, clss, id, inputChangeHandler, required, name, pattern = null, key }) => {
    ...
    return (
        <>
            ...
            {
                type !== "file" && type !== "range" && <input key={key} pattern={pattern} className={clss} name={name} defaultValue={defaultValue} required={required} onChange={inputChangeHandler} type={type} id={id} placeholder={placeholder} style={css} />
            }
        </>
    )
};
